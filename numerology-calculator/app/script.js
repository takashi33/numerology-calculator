(() => {
  ...
  const romajiHelpBtn = document.getElementById("romaji-help-btn");
  ...
  // 使い方の説明は、結果カードの「ⓘ」と同じヘルプモーダルに寄せる。タップ表を
  // 開くたびに長文が目に入ると圧迫感があるため、見たい人だけが開ける形にする。
  romajiHelpBtn.addEventListener("click", () => {
    openInfoModal({
      title: "ローマ字表の使い方",
      body: "苗字・名前どちらの欄も、ひらがな・カタカナで直接入力すると、少し待つと自動でヘボン式ローマ字に変換されます（例：「し」は shi）。うまく変換されない場合は、下のかなをタップして入力することもできます（直前にタップ・入力していた方の欄に追加されます）。同じ母音が続くときは2つ目をタップ（入力）しても文字は増えません（タップしたキーは光るので反応はわかります）。",
    });
  });
